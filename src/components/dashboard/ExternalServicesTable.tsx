      ...
      <table className="w-full text-sm">
        <thead className="p-[24px] border-b border-[#cfdbe8]">
          <tr className="text-[#0d141c]">
            ...
            <th className="text-left py-[12px] pr-[16px]">Details</th>
          </tr>
        </thead>
        <tbody>
          {services.map((s, i) => (
            <tr key={i} className="border-t text-[#0d141c] border-[#cfdbe8]">
              <td className="py-[24px] px-[16px] w-[350px]">{s.name}</td>
              <td className="py-[24px] w-[240px] px-[16px]">
                <button
...
"
              ...
              <td className="py-1 text-[#4a739c]">{s.details}</td>
            </tr>
          ))}
        </tbody>
      </table>
